param(
  [int]$StartYear = 1960,
  [int]$EndYear = 1989,
  [string]$OutYearDir = "imdb_sci_fi_year_files_primary_origin",
...
function Get-ImdbYearCandidates {
  ...
  do {
    $uri = "https://api.imdbapi.dev/titles?types=TV_SERIES&types=TV_MINI_SERIES&genres=Sci-Fi&startYear=$Year&endYear=$Year&minVoteCount=5000&sortBy=SORT_BY_USER_RATING&sortOrder=DESC"
    if ($pageToken) {
...
foreach ($year in $StartYear..$EndYear) {
  $yearCsv = Join-Path $OutYearDir "imdb_sci_fi_primary_origin_$year.csv"
  $yearJson = Join-Path $OutYearDir "imdb_sci_fi_primary_origin_$year.json"
  ...
  Write-Host "Wrote $($sortedRows.Count) primary-origin rows for $year."
}
